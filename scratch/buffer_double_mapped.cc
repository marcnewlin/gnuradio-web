...
bool buffer_double_mapped::allocate_buffer(int nitems)
{
    ...
    size_t buffer_size = d_bufsize * d_sizeof_item;
    printf("d_bufsize:     %lu\n"
           "d_sizeof_item: %lu\n"
           "buffer_size:   %lu\n", d_bufsize, d_sizeof_item, buffer_size);

    const size_t _page_size = 65536;
    if(buffer_size % _page_size > 0) {
      buffer_size += (_page_size - (buffer_size%_page_size));
    }
